...
describe('ResponseController', () => {
  ...
  it('should handle errors in fetching and saving data', async () => {
    ...
    (ResponseService.fetchAndSaveResponse as jest.Mock).mockRejectedValue(new AppError('Service Error'));

    const response = await request(app).post('/api/fetch');
    expect(response.status).toBe(500);
    expect(response.text).toContain("Error in fetching and saving response");
  });

  it('should handle errors in fetching historical data', async () => {
    // Mock ResponseService.getHistoricalData to throw an error
    (ResponseService.getHistoricalData as jest.Mock).mockRejectedValue(new AppError('Service Error'));

    const response = await request(app).get('/api/data');

    expect(response.status).toBe(500);
    expect(response.text).toContain("Error fetching historical data");
  });
});
