...
const RowDetailsModal: React.FC = () => {
  ...
  return (
    <Dialog open={modalState.open} onClose={handleCloseModal} maxWidth="md" fullWidth>
      <DialogTitle sx={{ fontWeight: "bold", fontSize: 24 }}>Row Details</DialogTitle>
      <DialogContent sx={{ padding: 3 }}>
        {/* Metadata Section */}
        <Box sx={{ marginBottom: "1.5rem", padding: 2, border: "1px solid #ddd", borderRadius: 2 }}>
          <Typography variant="h6" sx={{ fontWeight: "bold", marginBottom: "0.75rem" }}>
            <PriorityHigh sx={{ verticalAlign: "middle", marginRight: "8px" }} />
            Metadata
          </Typography>
          ...
          {tags && (
            <Typography variant="body1" sx={{ display: "flex", flexWrap: "wrap", gap: "0.5rem", marginTop: "0.75rem" }}>
              <strong>Tags:</strong>{" "}
              {tags.map((tag, index) => (
                <Chip key={index} label={tag} size="small" sx={{ margin: "0.2rem" }} icon={<Tag />} />
              ))}
            </Typography>
          )}
        </Box>
      ...
      <DialogActions sx={{ padding: "1rem" }}>
        <Button onClick={handleCloseModal} color="secondary" variant="outlined" sx={{ width: "100px" }}>
          Close
        </Button>
      </DialogActions>
    </Dialog>
